...
class dialog_traitements(QtWidgets.QDialog):
    ...
    def __init__(self, dict_indexes, dict_types, dict_paths, dict_mod, parent = None):
        ...
        self.vars_traites = dict.fromkeys(self.vars, False) #Dictionnaire des états des variables, False: non traitée, True : traitée.
        ...
        for base in self.l_bases: #base : tuple
            ...
            #Importation des bases ajoutées, renommage et conversion des variables. Calcul de l'âge, de l'âge quinquennal, ...
    ...
    def selectionner_input(self):
        ...
        elif self.current_input == coldict.STR_distrib_salaires_pensions_moy:
    ...
    def exporter_input(self):
        ...
        #current_date = datetime.now().strftime("%d%m%Y").replace('/', '_')
        #current_time = datetime.now().strftime("%H%M%S").replace(':', '_')

        #filename = self.current_input.replace(" ", "_") + ".csv"
        
        #self.df_output_to_export.to_csv(filename, sep = ";")
        self.df_output_to_export
